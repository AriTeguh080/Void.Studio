...
<head>
    ...
    <title>Abyss Whisper - VØIDX.STUDIO</title>
...
<body>
    ...
    <div class="container" style="margin-top:100px; text-align:center;">
        <div style="
        background: url('{{ asset('assets/images/awba.jpeg') }}') no-repeat center center/cover;
        color: white;
        padding: auto 0;
        width: 100vw;
    ...
    ">
        @php
            $article = \App\Models\Article::where('name', 'Abyss Whisper')->first();
        @endphp
        <br><h1>#2 {{ $article->name ?? 'Abyss Whisper' }}</h1><br><br><br>
        <img src="{{ asset('assets/images/awm.jpeg') }}" alt="Abyss Whisper" style="width:350px; border-radius:10px;"><br><br><br><br><br>
        <h3>{{ $article->slogan ?? 'The voice that calls from the edge of nothingness.' }}</h3><br>
        <p style="color: white; white-space: pre-line; word-wrap: break-word; max-width: 600px; margin: 0 auto;">
            {{ $article->description ?? 'Amidst the noise of the world, ABYSS WHISPER is a voice only heard by those who dare to be still.
It is the quiet call from the edge of nothingness not to frighten, but to remind us that even in the void, meaning speaks softly.
This piece embodies the feeling of sinking deep within one\'s own thoughts, yet finding guidance in the faint echoes that return.
A reflection of silence that speaks, and darkness that illuminates.' }}
        ...
        @auth
            <form method="POST" action="{{ route('favorite.add', ['article' => 2]) }}" style="display: inline;">
                @csrf
