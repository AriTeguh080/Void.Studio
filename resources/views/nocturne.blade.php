...
<head>
    ...
    <title>Nocturne - VØIDX.STUDIO</title>
...
<body>
    ...
    <div class="container" style="margin-top:100px; text-align:center;">
        <div style="
        background: url('{{ asset('assets/images/nrba.jpeg') }}') no-repeat center center/cover;
        color: white;
        padding: auto 0;
        width: 100vw;
    ...
    ">
        @php
            $article = \App\Models\Article::where('name', 'Nocturne')->first();
        @endphp
        <br><h1>#4 {{ $article->name ?? 'Nocturne' }}</h1><br><br><br>
        <img src="{{ asset('assets/images/nrm.jpeg') }}" alt="Nocturne" style="width:350px; border-radius:10px;"><br><br><br><br><br>
        <h3>{{ $article->slogan ?? 'The symphony of the night.' }}</h3><br>
        <p style="color: white; white-space: pre-line; word-wrap: break-word; max-width: 600px; margin: 0 auto;">
            {{ $article->description ?? 'NOCTURNE is a celebration of the night,
where darkness becomes a canvas for dreams and mysteries.
It invites you to embrace the quiet power of the unseen and the beauty
of what lies beyond the light. This piece captures the essence of midnight melodies,
where silence sings and shadows dance in harmonious rhythm.' }}
        </p>
        ...
        @auth
            <form method="POST" action="{{ route('favorite.add', ['article' => 4]) }}" style="display: inline;">
                @csrf
