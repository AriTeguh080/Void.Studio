...
<head>
    ...
    <title>Void Line - VØIDX.STUDIO</title>
...
<body>
    ...
    <div class="container" style="margin-top:100px; text-align:center;">
        <div style="
        background: url('{{ asset('assets/images/vlba.jpeg') }}') no-repeat center center/cover;
        color: white;
        padding: auto 0;
        width: 100vw;
    ...
    ">
        @php
            $article = \App\Models\Article::where('name', 'Void Line')->first();
        @endphp
        <br><h1>#1 {{ $article->name ?? 'Void Line' }}</h1><br><br><br>
        <img src="{{ asset('assets/images/vlm.jpeg') }}" alt="Void Line" style="width:350px; border-radius:10px;"><br><br><br><br><br>
        <h3>{{ $article->slogan ?? 'The boundary between existence and emptiness.' }}</h3><br>
        <p style="color: white; white-space: pre-line; word-wrap: break-word; max-width: 600px; margin: 0 auto;">
            {{ $article->description ?? 'Between every boundary lies an empty space — not meant to confine, but to liberate.
VØID LINE was born from the belief that true expression emerges from nothingness.
The cracked line between existence and void represents the courage to defy order, to stand apart, and to redefine the meaning of self within silence.
Every fracture tells a story; every imperfection breathes honesty.
Because only from emptiness, can freedom truly exist.' }}
        </p>
        ...
        @auth
            <form method="POST" action="{{ route('favorite.add', ['article' => 1]) }}" style="display: inline;">
                @csrf
